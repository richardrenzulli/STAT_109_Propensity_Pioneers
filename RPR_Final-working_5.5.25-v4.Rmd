...
```{r,eval=TRUE, fig.cap="Upsample and set up splits"}
...
prepare_named_splits <- function(data,
  ...
  for (split_name in names(split_defs)) {
    ...
    # Manage named output
    message("Created: ", train_var, ", ", test_var)
  }
}
...
train_mc_2_50 <- total_data_train_50 %>%
...
train_mc_2_60 <- total_data_train_60 %>%
  mutate(is_computer = as.factor(ifelse(device_computer == 1, 1, 0)),
         is_tablet = as.factor(ifelse(device_tablet == 1, 1, 0))) %>%
  select(-device_mobile, -device_computer, -device_tablet)
...
train_mc_2_70 <- total_data_train_70 %>%
  mutate(is_computer = as.factor(ifelse(device_computer == 1, 1, 0)),
         is_tablet = as.factor(ifelse(device_tablet == 1, 1, 0))) %>%
  select(-device_mobile, -device_computer, -device_tablet)
...
train_mc_2_80 <- total_data_train_80 %>%
  mutate(is_computer = as.factor(ifelse(device_computer == 1, 1, 0)),
         is_tablet = as.factor(ifelse(device_tablet == 1, 1, 0))) %>%
  select(-device_mobile, -device_computer, -device_tablet)
